...
export function EmptyState({
  ...
  return (
    <div className="grid place-items-center rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
      <span className="grid size-14 place-items-center rounded-2xl bg-violet-50 text-violet-600 ring-1 ring-violet-100">
      ...
      {actionLabel && actionHref ? (
        <Link
          href={actionHref}
          className="focus-ring mt-5 inline-flex min-h-11 items-center justify-center rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-violet-600"
        >
          {actionLabel}
        </Link>
      ) : null}
